-- SECURITY FIX: Secure push_events table from unauthorized access

-- First, let's examine and fix the push_events RLS policies
-- The issue is likely that the system update policy is too permissive
...
-- Add security audit logging for push event access
CREATE OR REPLACE FUNCTION public.log_push_event_access()
RETURNS TRIGGER AS $$
BEGIN
  -- Log when push events are accessed
...
CREATE TRIGGER push_events_audit_trigger
  AFTER INSERT OR UPDATE OR DELETE OR SELECT ON public.push_events
  FOR EACH ROW EXECUTE FUNCTION public.log_push_event_access();

-- Add index for better performance on user_id queries (common access pattern)
