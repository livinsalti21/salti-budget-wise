...
const NetWorthProjection = ({
  ...
  useEffect(() => {
    if (ledgerHistory && ledgerHistory.length > 0) {
      ...
      if (saveEntries.length > 0) {
        ...
        const recentSaves = saveEntries.filter(entry => new Date(entry.created_at) >= thirtyDaysAgo);
  ...
  const calculateProjections = () => {
    ...
    const calculateFutureValue = (years: number) => {
      ...
      return currentSavingsValue * Math.pow(1 + interestRate / 100, years) + monthlyContribution * (Math.pow(1 + monthlyRate, months) - 1) / monthlyRate;
    };
    return {
  ...
  const monthsWithoutInterest = targetAmount > currentSavings && monthlyContribution > 0 ? Math.ceil((targetAmount - currentSavings) / monthlyContribution) : 0;
  const monthsSaved = monthsToGoal > 0 && monthsToGoal !== Infinity ? monthsWithoutInterest - monthsToGoal : 0;
      ...
      <CardContent className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 p-6 bg-muted/20 rounded-xl">
          <div className="space-y-2">
            ...
            <Slider id="monthly-contribution" min={50} max={2000} step={25} value={[monthlyContribution]} onValueChange={value => setMonthlyContribution(value[0])} className="w-full" />
          ...
          <div className="space-y-2">
            ...
            <Slider id="time-horizon" min={1} max={50} step={1} value={[timeHorizon]} onValueChange={value => setTimeHorizon(value[0])} className="w-full" />
            <div className="flex justify-between text-xs text-muted-foreground">
              <span>1yr</span>
              <span>50yrs</span>
            </div>
          ...
          <div className="space-y-2">
            ...
            <Input id="target-amount" type="number" value={targetAmount} onChange={e => setTargetAmount(Number(e.target.value))} className="text-xl font-bold h-12" />
          </div>
          ...
          <div className="space-y-2">
            ...
            <Slider min={1} max={15} step={0.5} value={[interestRate]} onValueChange={value => setInterestRate(value[0])} className="w-full mt-5" />
            <div className="flex gap-2 flex-wrap">
              {ratePresets.map(preset => <Button key={preset.label} variant={interestRate === preset.rate ? "default" : "outline"} size="sm" onClick={() => setInterestRate(preset.rate)} className="text-xs flex-1">
                  {preset.rate}%
                </Button>)}
            </div>
          </div>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={400}>
              <LineChart data={generateChartData}>
                ...
                <Legend />
                <Line type="monotone" dataKey="totalValue" stroke="hsl(var(--primary))" strokeWidth={3} name="Total Value" dot={false} />
                <Line type="monotone" dataKey="contributions" stroke="hsl(var(--muted-foreground))" strokeWidth={2} strokeDasharray="5 5" name="Your Contributions" dot={false} />
                <Line type="monotone" dataKey="growth" stroke="hsl(var(--success))" strokeWidth={2} name="Interest Growth" dot={false} />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <Card className="shadow-sm">
            ...
            <CardContent className="space-y-4">
              {monthsToGoal === -1 ? <div className="text-center py-4">
                ...
                </div> : monthsToGoal === Infinity ? <div className="text-center py-4">
                  ...
                  {interestRate > 0 && monthsToGoal < monthsWithoutInterest && <div className="p-4 bg-success/10 rounded-lg space-y-2 border border-success/20">
