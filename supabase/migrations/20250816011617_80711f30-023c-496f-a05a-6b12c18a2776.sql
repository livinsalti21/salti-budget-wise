-- Livin Salti Save n Stack - Complete Database Schema
-- User profiles for additional data beyond auth
...
-- Savings records with enhanced future value projections
CREATE TABLE IF NOT EXISTS public.saves (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  amount_cents INTEGER NOT NULL CHECK (amount_cents > 0),
  reason TEXT NOT NULL,
  source TEXT DEFAULT 'manual' CHECK (source IN ('manual', 'rule', 'match')),
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);

-- Future value projections for each save
...
-- Save matches for social saving
...
-- Budget templates and purchases
...
-- Functions for projection calculations
