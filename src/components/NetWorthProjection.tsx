...
const NetWorthProjection = ({
  ...
  const calculateProjections = () => {
    ...
    const fiveYearValue = currentSavings * Math.pow(1 + interestRate / 100, 5) + monthlyContribution * (Math.pow(1 + monthlyRate, 60) - 1) / monthlyRate;
    return {
  ...
  const getScenarioProjection = (extraMonthly: number) => {
    ...
    const monthlyRate = interestRate / 100 / 12;
    return currentSavings * Math.pow(1 + interestRate / 100, 5) + totalMonthly * (Math.pow(1 + monthlyRate, 60) - 1) / monthlyRate;
  };
  ...
  return <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input id="target-amount" type="number" value={targetAmount} onChange={e => setTargetAmount(Number(e.target.value))} className="text-lg font-medium" />
            </div>

            <div className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor="interest-rate">Expected Annual Return: {interestRate}%</Label>
                <Slider id="interest-rate" min={1} max={15} step={0.5} value={[interestRate]} onValueChange={value => setInterestRate(value[0])} className="w-full" />
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 gap-2">
                  {getInvestmentType(interestRate).map((type, index) => <div key={index} className={`p-2 rounded-lg border text-xs ${type.active ? 'bg-primary/10 border-primary/30' : 'bg-muted/20'}`}>
                      ...
                      {type.active && <p className="text-muted-foreground mt-1">{type.description}</p>}
                    </div>)}
                </div>
              </div>
            </div>
      ...
      {/* Budget Insights */}
      {budgetAnalysis && insights.length > 0 && <Card className="border-accent/20 bg-gradient-to-r from-accent/5 to-accent/10">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
          ...
          <CardContent>
            <div className="space-y-3">
              {insights.map((insight, index) => <div key={index} className="flex items-start gap-2 p-3 rounded-lg bg-background/50">
