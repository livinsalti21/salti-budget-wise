...
serve(async (req) => {
  const logger = new EdgeFunctionLogger('payday-runner');
  ...
  try {
    logger.info("Payday runner triggered");
    ...
    const now = new Date();
    logger.debug("Current time", { now: now.toISOString() });
    ...
    if (!dueRules || dueRules.length === 0) {
      logger.info("No rules due for execution");
    ...
    logger.info("Found due rules", { count: dueRules.length });
    ...
    for (const rule of dueRules) {
      try {
        logger.info("Processing rule", { rule_id: rule.id, user_id: rule.user_id });
        ...
        logger.info("Created save event", { save_event_id: saveEvent.id });
        ...
        logger.info("Updated rule next run", { rule_id: rule.id, next_run: nextRun.toISOString() });
        ...
        try {
          ...
          if (!response.ok) {
            logger.warn("Match engine call failed", { status: response.status });
          } else {
            logger.info("Match engine triggered successfully");
          }
        } catch (matchError) {
          logger.error("Error calling match engine", matchError);
        }
    ...
    logger.info("Payday runner completed", { executedCount, errorCount });
    ...
    logger.error("ERROR in payday-runner", { message: errorMessage });
