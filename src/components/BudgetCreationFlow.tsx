...
import { Sparkles, Upload, ShoppingBag, Calculator, ArrowRight, FileSpreadsheet, Crown, Zap } from 'lucide-react';
...
const BudgetCreationFlow = ({
  onMethodSelected,
  onBudgetCreated
}: BudgetCreationFlowProps) => {
  ...
  const methods = [...(isFallbackEnabled ? [{
    ...
    examples: ['Student budgets with 35% groceries allocation', 'Family budgets with 40% groceries allocation', 'Automatic savings optimization']
  }] : []), ...(isAIEnabled ? [{
    id: 'ai' as const,
    ...
    examples: ['Step-by-step income entry', 'Categorized expense tracking', 'Goal-setting with timelines']
  }];
  ...
  return <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        return <Card key={method.id} className={cn("cursor-pointer transition-all duration-200 hover:shadow-md hover:-translate-y-1", method.color)} onClick={() => handleMethodSelect(method)}>
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3">
                    <div className={cn("p-2 rounded-lg bg-background/50", `${method.iconColor}`)}>
