-- Security fixes: Remove hardcoded secrets and improve encryption
...
-- Fix RLS policies for push_events table (ensure proper access control)
-- First, let's check if the table exists and create proper policies
DO $$
BEGIN
  ...
  DROP POLICY IF EXISTS "System can manage push events" ON public.push_events;
  
  -- Create proper RLS policies for push_events
  ...
  CREATE POLICY "System can update push events"
    ON public.push_events
    FOR UPDATE
    USING (true); -- System needs to update status, but we'll add app-level controls
...
EXCEPTION
  WHEN undefined_table THEN
    -- Table doesn't exist, skip policy creation
    NULL;
END $$;
